## LD.heat.map -- Steven J. Mack February 20-27, 2019 v0.2
...
#' @references Kazutoyo et al. Hum Immunol. 2019 TBA

LD.heat.map <- function(dataName,color=TRUE) {
...
if(!dTop & !dBot) { 
  cat(paste("Files ",dataName,"_Phased_LD_results.csv"," and ",dataName,"_Unphased_LD_results.csv"," are not found.",sep=""))
  } else {
...
if(dTop) { phaseDataRaw <- phaseDataRaw[!is.na(phaseDataRaw$N_Haplotypes),]}    
...
for(i in 1:4) {
  ...
  phaseDataRaw[[i+1]] <- as.numeric(phaseDataRaw[[i+1]])
  unphaseDataRaw[[i+1]] <- as.numeric(unphaseDataRaw[[i+1]])
