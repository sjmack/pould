...
### April 28, 2026 v1.0.1 -- Steven J. Mack,
##-----------------------------------------------------------------------------------------------------------------------------------------##
...
##    The Gl String column should consist of two tilde (~) delimited haplotypes connected by a plus (+) sign
...
LDWrap <- function(famData,threshold=10,phased=TRUE,frameName="hla-family-data",trunc=0,writeTo=tempdir()){
  ...
  for(i in 1:nSamp) {
    ...
    for(j in 1:nLoc1) {
       if(substr(hap1[j],1,4) %in% c("HLA-")) { ## check to see if this using an HLA-prefix; otherwise assume an internal "-" is intentional for a position
       currLoc <- substr(hap1[j],regexpr("-",text = hap1[j],fixed=TRUE)[1]+1,regexpr("*",text = hap1[j],fixed=TRUE)[1]-1) } else {
         currLoc <- substr(hap1[j],1,regexpr("*",text = hap1[j],fixed=TRUE)[1]-1)
       }
    ...
    for(j in 1:nLoc2) {
      if(substr(hap2[j],1,4) %in% c("HLA-")) {
      currLoc <- substr(hap2[j],regexpr("-",text = hap2[j],fixed=TRUE)[1]+1,regexpr("*",text = hap2[j],fixed=TRUE)[1]-1) } else {
        currLoc <- substr(hap2[j],1,regexpr("*",text = hap2[j],fixed=TRUE)[1]-1)
      }
...
#' The implementation of ALD applied here is calculated using individual Dij LD values and allele frequencies.
#'
...
#' @param verbose A boolean identifying if results should be printed to the console (verbose = TRUE), or returned in a vector of (D', Wn, WLocus2/Locus1, WLocus1/Locus2, number of haplotypes) (verbose = FALSE)
