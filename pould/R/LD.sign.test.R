## Performing sign tests on Phased and UnPhased LD values -- Steven J. Mack June 4-6, 2018 v0.01

#' Perform the sign test on paired LD values for phased and unphased haplotypes 
#' 
#' Applies binom.test() to pairs of \eqn{D'}, \eqn{Wn}, \eqn{WLoc1/Loc2}, \eqn{WLoc2/Loc1} and number of haplotypes values in the *_LD_results.csv files generated by LDWrap(). 
#' 
...
#' @param returnFrame A boolean identifying if a data frame of results should be returned (returnFrame=TRUE). If returnFrame=FALSE, a CSV file of results named <dataName>_LD-sign-test_results.csv is written in the working directory.
#' @keywords sign test linkage disequilibrium
#' @importFrom utils write.table read.table read.csv
#' @importFrom stats binom.test
#' @return A data frame of five columns (\eqn{D'}, \eqn{Wn}, \eqn{WLoc1/Loc2}, \eqn{WLoc2/Loc1} and N_Haplotypes) and four rows (#unphased > phased, #unphased = phased, #locus pairs and p-values).
...
phaseDataRaw <- read.csv(file=paste(dataName,"_Phased_LD_results.csv",sep=""),header = T)
unphaseDataRaw <- read.csv(file=paste(dataName,"_Unphased_LD_results.csv",sep=""),header=T)
...
if (returnFrame) {return(result)}
  ...
  if(!file.exists(paste(dataName,"_Phased_LD_results.csv",sep="")) && !file.exists(paste(dataName,"_Unphased_LD_results.csv",sep=""))) {
      cat(paste("LD.sign.test: Files ",dataName,"_Phased_LD_results.csv"," and ",dataName,"_Unphased_LD_results.csv"," are not found.",sep=""))
  } else {if(!file.exists(paste(dataName,"_Phased_LD_results.csv",sep=""))) { 
      cat(paste("LD.sign.test: File ",dataName,"_Phased_LD_results.csv"," is not found.",sep=""))
      } else {cat(paste("LD.sign.test: File ",dataName,"_Unhased_LD_results.csv"," is not found.",sep=""))}
    }
